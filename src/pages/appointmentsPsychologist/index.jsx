      ...
      <Container>
        <FiltersContainer>
          <SearchContainer>
            ...
            <SearchBar placeholder="Buscar paciente..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </SearchContainer>
            ...
            <Stack alignItems="flex-start" spacing={2}>
              {filteredAppointments?.slice((page - 1) * pageSize, page * pageSize).map((consultation) => (
                <ConsultationCard key={consultation.id}>
                  <PatientPictureContainer>
                    <PatientPicture src={consultation.patientPicture} alt={consultation.patient} />
                  </PatientPictureContainer>
                  <ConsultationInfo>
                  ...
                  <IconButton size="small" onClick={(e) => handleMenuOpen(e, consultation)} sx={{ color: Colors.GREY, p: '6px' }} aria-controls={menuOpen ? 'psychologist-appointment-menu' : undefined} aria-haspopup="true" aria-expanded={menuOpen ? 'true' : undefined}>
                    <MoreHoriz />
                  </IconButton>
                </ConsultationCard>
              ))}
        ...
        >
          {selectedConsultation?.status === 'Aguardando pagamento' && (
            <MenuItem onClick={() => { handleMenuClose(); navigate('/payment', { state: { psychologist: { name: selectedConsultation.patient, picture: selectedConsultation.patientPicture, crp: selectedConsultation.crp || '', rating: selectedConsultation.rating || 4.5, specialties: selectedConsultation.specialties || [], approaches: selectedConsultation.approaches || [] }, slot: { datetime: selectedConsultation.datetime }, price: selectedConsultation.price || 150 } }); }}>
              <ListItemIcon>
          ...
          {selectedConsultation?.status === 'Confirmada' && new Date(selectedConsultation?.datetime) < new Date() && (
            <MenuItem onClick={() => { handleMenuClose(); navigate('/payment', { state: { psychologist: { name: selectedConsultation.patient, picture: selectedConsultation.patientPicture, crp: selectedConsultation.crp || '', rating: selectedConsultation.rating || 4.5, specialties: selectedConsultation.specialties || [], approaches: selectedConsultation.approaches || [] }, slot: { datetime: selectedConsultation.datetime }, price: selectedConsultation.price || 150 } }); }}>
              <ListItemIcon>
          ...
          <MenuItem onClick={() => { handleMenuClose(); console.log('Cancelar', selectedConsultation?.id); }}>
            <ListItemIcon>
